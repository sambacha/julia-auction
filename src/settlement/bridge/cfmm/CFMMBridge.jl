...
function get_route(bridge::Bridge{T}, token_in::Int64, token_out::Int64, amount_in::T, slippage::T) where {T}
    # Check cache first
    ...
    try
        ...
        try
            ...
            return RoutingResult(price, amount_out, path, pools, price_impact, gas_estimate)
        catch router_error
...
function execute_direct(bridge::Bridge{T}, token_in::Int64, token_out::Int64, amount_in::T, slippage::T) where {T}
    # Get route first
    ...
    try
        ...
        return ExecutionResult(actual_price, actual_amount, gas_used, tx_hash)
    catch e
