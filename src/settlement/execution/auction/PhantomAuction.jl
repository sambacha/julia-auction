...
mutable struct Auction{T}
    ...
    function Auction{T}(
        ...
        global_config::AuctionConfig = load_config(),
    ) where {T}
        new{T}(
            config,
            baseline_price,
...
function validate_bid(auction::Auction{T}, bid::ImprovementBid{T}) where {T}
    ...
    if improvement_bps < auction.config.min_improvement_bps || improvement_bps > auction.config.max_improvement_bps
        return false
    end
...
function create_result(auction::Auction{T}) where {T}
    ...
    improvement_bps = calculate_improvement_bps(auction.baseline_price, auction.winning_bid.improved_price)
...
"""
function configure_auction_bid_source(auction::Auction, participant_endpoints::Vector{String})
...
function create_blockspace_commitment(bid::BlockspaceImprovementBid{T}) where {T}
    data = string(bid.bidder_id, bid.slot_number, bid.improved_block_value, bid.mev_component, bid.nonce)
    return sha256(data)
end
