...
struct SimulationConfig
    ...
    function SimulationConfig(;
        ...
        new(num_real_bidders, num_phantom_bidders, auction_duration_ms,
            num_simulations, seed, enable_vcg, enable_learning)
    end
end
...
"""
function run_simulation(simulator::PhantomSimulator, 
    ...
    metrics = calculate_simulation_metrics(
        all_bids, winning_bid, clearing_price, baseline_price,
        execution_time, convergence_path
    )
...
"""
function run_monte_carlo(simulator::PhantomSimulator,
                        baseline_price::Float64,
                        baseline_quantity::Float64)
    results = SimulationResult[]
    
    for i in 1:simulator.config.num_simulations
...
"""
function generate_real_bids(simulator::PhantomSimulator,
                           baseline_price::Float64,
                           baseline_quantity::Float64)
    bids = SimulatedBid[]
    
    for i in 1:simulator.config.num_real_bidders
        # Real bidders follow informed distribution
        improvement_bps = rand(simulator.rng, 
            Normal(simulator.calibration_params["bid_mean"], 
                  simulator.calibration_params["bid_std"]))
...
"""
function generate_phantom_bids(simulator::PhantomSimulator,
                             baseline_price::Float64,
                             baseline_quantity::Float64)
    bids = SimulatedBid[]
    
    for (i, strategy) in enumerate(simulator.strategies[1:simulator.config.num_phantom_bidders])
        bid = generate_bid(strategy, baseline_price, baseline_quantity, 
                          simulator.market, simulator.rng)
...
"""
function calculate_simulation_metrics(bids::Vector{SimulatedBid},
...
function analyze_results(results::Vector{SimulationResult})
    ...
    analysis["strategy_performance"] = Dict(
        k => v / total_auctions for (k, v) in strategy_wins
    )
...
"""
function calculate_total_surplus(bids::Vector{SimulatedBid}, 
                                clearing_price::Float64,
                                baseline_price::Float64)
    consumer_surplus = sum([
        max(0, b.price - clearing_price) * b.quantity 
        for b in bids if b.price >= clearing_price
    ])
    
    producer_surplus = (clearing_price - baseline_price) * 
                      sum([b.quantity for b in bids if b.price >= clearing_price])
...
"""
function backtest_strategy(simulator::PhantomSimulator, 
                          strategy::BidderStrategy,
                          historical_scenarios::Vector{Dict})
    results = []
    ...
    for scenario in historical_scenarios
        ...
        bid = generate_bid(strategy, baseline_price, baseline_quantity,
                          market_conditions, simulator.rng)
        
        # Compare to actual outcome
        actual_clearing = scenario["actual_clearing_price"]
        
        if bid !== nothing
            profit = bid.price >= actual_clearing ? 
                    (bid.price - actual_clearing) * bid.quantity : 0.0
