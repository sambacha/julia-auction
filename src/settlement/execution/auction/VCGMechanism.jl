...
struct VCGBid{T<:NumericValue}
    ...
    function VCGBid(bidder_id::String, valuation::T, quantity::T=one(T); 
                    is_phantom::Bool=false, metadata::Dict{Symbol, Any}=Dict{Symbol, Any}()) where T<:NumericValue
...
mutable struct VCGAuction{T<:NumericValue}
    ...
    function VCGAuction(supply::T, reserve_price::T=zero(T); 
                       enable_multi_unit::Bool=false) where T<:NumericValue
...
function run_vcg_auction(auction::VCGAuction{T}) where T<:NumericValue
    ...
    if isempty(eligible_bids)
        return VCGResult{T}(
            ...
            zero(T),
            0.0,
            calculate_phantom_rate(auction.bids)
        )
    end
    ...
    for (bidder_id, quantity) in allocation_map
        ...
        if bid !== nothing
            ...
            push!(allocations, VCGAllocation{T}(
                bidder_id,
                quantity,
                payment,
                utility
...
"""
function find_optimal_allocation(bids::Vector{VCGBid{T}}, supply::T, 
                                multi_unit::Bool) where T<:NumericValue
...
"""
function calculate_payments(bids::Vector{VCGBid{T}}, 
                           allocation::Dict{String, T},
                           supply::T) where T<:NumericValue
...
"""
function calculate_welfare(bids::Vector{VCGBid{T}}, 
                          allocation::Dict{String, T}) where T<:NumericValue
    welfare = zero(T)
...
"""
function verify_truthfulness(auction::VCGAuction{T}, bidder_id::String, 
                            true_value::T) where T<:NumericValue
...
"""
function update_bid!(auction::VCGAuction{T}, bidder_id::String, 
                    new_value::T) where T<:NumericValue
    bid_idx = findfirst(b -> b.bidder_id == bidder_id, auction.bids)
    
    if bid_idx !== nothing
        # Update existing bid
        old_bid = auction.bids[bid_idx]
        auction.bids[bid_idx] = VCGBid(
            bidder_id,
            new_value,
            old_bid.quantity,
            is_phantom=old_bid.is_phantom,
            metadata=old_bid.metadata
        )
    else
        # Add new bid
