...
"""
function generate_bid(strategy::RandomWalkStrategy,
    ...
    drift_component = strategy.mean_reversion_speed * 
                     (strategy.long_run_mean - current_improvement) * dt
    ...
    improvement_bps = current_improvement + drift_component + 
                     diffusion_component * current_improvement + jump_component
...
"""
function simulate_path(strategy::RandomWalkStrategy, n_steps::Int, 
                      dt::Float64, rng::Random.AbstractRNG)
    ...
    for i in 2:(n_steps + 1)
        ...
        # Jump
        jump = rand(rng) < strategy.jump_intensity * dt ? 
               strategy.jump_size_mean * randn(rng) : 0.0
