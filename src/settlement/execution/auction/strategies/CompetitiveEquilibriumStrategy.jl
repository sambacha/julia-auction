...
function generate_bid(
    ...
    timestamp =
        strategy.information_precision > 0.8 ? rand(rng, Exponential(10.0)) :  # Bid early
    ...
    return (price = price, quantity = quantity, timestamp = timestamp, priority_fee = priority_fee)
end
...
"""
function solve_equilibrium(strategy::CompetitiveEquilibriumStrategy, market::MarketConditions, n_iterations::Int = 100)
...
"""
function best_response(opponent_strategy::Function, strategy::CompetitiveEquilibriumStrategy, market::MarketConditions)
...
function expected_payoff(
    ...
    risk_adjustment =
        strategy.risk_aversion > 0 ? (1 - exp(-strategy.risk_aversion * payoff_if_win)) / strategy.risk_aversion :
        payoff_if_win
