...
function process_settlement(
    ...
    try
        ...
        # Phase 2: Execution
        execution_result = _execute_settlement(
            orchestrator,
            transaction,
            preparation_result,
        )
...
function _prepare_settlement(
    ...
    routes = if !isnothing(cached_routes)
        ...
        routes = RouteOptimizer.find_optimal_routes(
    ...
    protection_strategy = MEVProtection.select_protection(
        orchestrator.mev_protector,
        mev_risk,
    )
    
    protected_routes = MEVProtection.apply_protection(
        orchestrator.mev_protector,
        routes,
        protection_strategy,
    )
...
function _execute_settlement(
    ...
    commit_coordinator = AtomicExecution.create_coordinator(
        orchestrator.executor,
        transaction,
    )
    
    # Phase 2a: Prepare all participants
    prepare_results = AtomicExecution.prepare_phase(
        commit_coordinator,
        preparation_result.routes,
    )
...
function _update_metrics(
    ...
    SettlementMonitor.record_settlement(
...
function emergency_halt(orchestrator::SettlementOrchestrator)
    lock(orchestrator.state_lock) do
        ...
        # Notify monitor
        SettlementMonitor.trigger_alert(
            orchestrator.monitor,
            "EMERGENCY_HALT",
            "Manual emergency halt activated",
        )
    end
end
...
"""
function _create_receipt(
    transaction::SettlementTransaction{T},
    commit_results,
)::SettlementReceipt{T} where {T<:Real}
...
function _handle_settlement_error(orchestrator::SettlementOrchestrator, error)
    ...
    SettlementMonitor.record_error(
        orchestrator.monitor,
        string(error),
