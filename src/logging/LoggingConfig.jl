...
function Logging.handle_message(
    ...
    log_line = "$(timestamp) $(level_color)$(level_str)$(reset_color)" *
...
"""
function create_json_formatter(;
...
"""
function create_console_formatter(;
    min_level::LogLevel = Logging.Info,
    stream::IO = stderr
)::ConsoleFormatter
...
function configure_logging!(;
    ...
    formatter = if format == :json || 
                  (format == :auto && environment == :production)
