...
export BlockspaceBid,
    BlockspaceAuction,
    BlockspaceAuctionResult,
    create_blockspace_auction,
    submit_bid,
    run_blockspace_auction,
    allocate_slot,
    calculate_mev_distribution
...
function submit_bid(auction::BlockspaceAuction{T}, bid::BlockspaceBid{T})::Bool where {T<:Real}
...
function run_blockspace_auction(auction::BlockspaceAuction{T})::BlockspaceAuctionResult{T} where {T<:Real}
    start_time = now()
...
function calculate_mev_distribution(mev_value::T, config::BlockspaceAuctionConfig)::Tuple{T,T} where {T<:Real}
