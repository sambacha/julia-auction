...
export EthereumClient, BlockHeader, ExecutionPayload, PayloadStatus,
       connect_ethereum, get_latest_block, get_payload, new_payload,
       forkchoice_updated, get_validator_duties, register_validator
...
function test_connection(client::EthereumClient)
    ...
    consensus_response = HTTP.get(
        "$(client.consensus_endpoint)/eth/v1/node/version",
...
function get_current_slot(client::EthereumClient)::Int64
    ...
    response = HTTP.get(
        "$(client.consensus_endpoint)/eth/v1/beacon/headers/head",
        timeout=client.timeout_ms/1000
    )
...
function get_block_by_slot(
    ...
    try
        response = HTTP.get(
            "$(client.consensus_endpoint)/eth/v2/beacon/blocks/$slot",
            timeout=client.timeout_ms/1000
        )
...
function get_sync_status(client::EthereumClient)::Dict{String, Any}
    ...
    response = HTTP.get(
        "$(client.consensus_endpoint)/eth/v1/node/syncing",
        timeout=client.timeout_ms/1000
    )
