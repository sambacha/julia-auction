...
export Validator, ValidatorRegistry, SlashEvent,
       register_validator, update_validator, get_validator,
       is_validator_active, slash_validator, update_reputation,
       get_validators_for_epoch, rotate_proposer_duties
...
function slash_validator(
    ...
    lock(registry.registry_lock) do
        ...
        # Update reputation
        reputation_penalty = slash_type == :double_sign ? 0.5 :
                           slash_type == :mev_theft ? 0.3 :
                           slash_type == :surround_vote ? 0.4 : 0.1
...
function update_reputation(
    ...
    lock(registry.registry_lock) do
        ...
        consistency_score = validator.missed_proposals == 0 ? 0.3 : 
...
function get_validators_for_epoch(
    registry::ValidatorRegistry,
    epoch::Int64
)::Vector{Validator}
...
function get_validator_for_slot(
    registry::ValidatorRegistry,
    slot::Int64
)::Union{Nothing, Validator}
...
function export_validator_stats(
    registry::ValidatorRegistry
)::Dict{String, Any}
    ...
    lock(registry.registry_lock) do
        stats = Dict{String, Any}(
            ...
            "average_reputation" => mean([v.reputation_score for v in values(registry.validators) if v.pubkey in registry.active_validators]),
            "timestamp" => now()
        )
