...
function create_slot_schedule(start_epoch::Int64, end_epoch::Int64; slots_per_epoch::Int64 = 32)::SlotSchedule
    schedule_id = uuid4()
...
function priority_allocation(
    ...
    normal_validators =
        filter(v -> v[2] < strategy.priority_threshold * 32.0 && v[2] >= strategy.min_validator_stake, validators)
...
function get_slot_allocation(schedule::SlotSchedule, slot_number::Int64)::Union{Nothing,SlotAllocation}
...
function calculate_allocation_metrics(schedule::SlotSchedule)::Dict{String,Any}
    ...
    avg_gas_used = isempty(executed_allocations) ? 0.0 : mean([a.gas_used for a in executed_allocations])
