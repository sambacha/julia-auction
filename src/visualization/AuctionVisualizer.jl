...
"""
function plot_bid_progression(
    data::AuctionVisualizationData;
    show_mean = true,
    show_range = true,
...
"""
function plot_revenue_efficiency(data::AuctionVisualizationData; title = "Revenue and Efficiency Analysis")
    ...
    if length(data.revenues) >= 10
        ma_window = 10
        ma_revenue = [mean(data.revenues[max(1, i-ma_window+1):i]) for i = 1:length(data.revenues)]
        plot!(p1, data.rounds, ma_revenue, label = "10-Round MA", color = :navy, linewidth = 2, linestyle = :dash)
    end
    ...
    hline!(p2, [0.9], label = "90% Threshold", color = :red, linestyle = :dash, linewidth = 1)
...
"""
function plot_winner_distribution(data::AuctionVisualizationData; title = "Winner Distribution")
    ...
    for (i, (id, count)) in enumerate(zip(bidder_ids, win_counts))
        ...
        annotate!(p, id, count, text("$(percentage)%", :center, 8))
    end

    # Add average line
    avg_wins = mean(win_counts)
    hline!(p, [avg_wins], label = "Average", color = :red, linestyle = :dash, linewidth = 2)
...
"""
function plot_competition_analysis(data::AuctionVisualizationData; title = "Competition Analysis")
    ...
    # Create subplots
    p1 = plot(
        data.rounds[1:length(bid_spreads)],
        bid_spreads,
    ...
    if length(avg_bids) > 1
        ...
        plot!(p2, avg_bids, trend_line, label = "Trend", color = :red, linewidth = 2, linestyle = :dash)
    end
...
"""
function plot_auction_comparison(
    data_list::Vector{AuctionVisualizationData};
    ...
    if length(plots_array) > 1
        p = plot(plots_array..., layout = (1, length(plots_array)), size = (400*length(plots_array), 400))
    else
        p = plots_array[1]
    end
...
"""
function plot_bidder_performance(
    data::AuctionVisualizationData,
    bidder_id::Int;
    ...
    if isempty(bidder_bids)
        return plot(title = "No data for Bidder $bidder_id", grid = false, showaxis = false)
    end

    # Plot bid history
    p1 = plot(
        1:length(bidder_bids),
        bidder_bids,
        title = title,
        ylabel = "Bid Value",
        xlabel = "Round",
    ...
    if !isempty(bidder_wins)
        ...
        scatter!(p1, bidder_wins, win_bids, label = "Wins", color = :gold, markersize = 8, markershape = :star)
    end
    ...
    for i = window_size:length(data.rounds)
    ...
    if !isempty(win_rates)
        p2 = plot(
            window_size:length(data.rounds),
            win_rates,
            ylabel = "Win Rate",
            xlabel = "Round",
...
"""
function create_auction_dashboard(data::AuctionVisualizationData; save_path = nothing)
