...
mutable struct SimulatedValidator <: AbstractParticipantValidator
    ...
    function SimulatedValidator(;
        ...
        new(base_success_rate, failure_escalation_rate, Dict{UUID,Float64}(), response_delay_range)
    end
end
...
"""
function simulate_participant_decision(
    ...
    if !decision
        ...
        validator.participant_reliability[participant_id] = max(0.1, current_reliability - 0.01)
    else
        # Slightly improve reliability on success
        validator.participant_reliability[participant_id] = min(1.0, current_reliability + 0.005)
    end
...
"""
function ParticipantDecisionInterface.validate_transaction(
    validator::SimulatedValidator,
    ...
    decision = simulate_participant_decision(
        ...
        failure_history = request.retry_attempt,
    )
...
"""
function configure_failure_rates!(validator::SimulatedValidator, participant_rates::Dict{UUID,Float64})
...
"""
function simulate_network_partition!(validator::SimulatedValidator, partitioned_participants::Vector{UUID})
...
function get_simulation_stats(validator::SimulatedValidator)
    if isempty(validator.participant_reliability)
        return Dict(:tracked_participants => 0, :average_reliability => validator.base_success_rate)
    end

    avg_reliability = sum(values(validator.participant_reliability)) / length(validator.participant_reliability)
