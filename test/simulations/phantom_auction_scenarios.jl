...
@testset "Phantom Auction Simulations" begin
    @testset "Basic Simulation Functionality" begin
        ...
        config = SimulationConfig(num_real_bidders = 3, num_phantom_bidders = 7, auction_duration_ms = 100, seed = 123)

        market = MarketConditions(volatility = 0.2, liquidity = 0.7, trend = 0.0)
    ...
    @testset "VCG Mechanism Integration" begin
        config = SimulationConfig(num_real_bidders = 5, num_phantom_bidders = 10, enable_vcg = true, seed = 456)
    ...
    @testset "Market Condition Scenarios" begin
        @testset "High Volatility Market" begin
            ...
            simulator = PhantomSimulator(config, market)
            results = [run_simulation(simulator, 100.0, 1000.0) for _ = 1:10]
    ...
    @testset "Strategy Performance Tests" begin
        config = SimulationConfig(num_real_bidders = 2, num_phantom_bidders = 15, num_simulations = 100, seed = 303)
    ...
    @testset "Edge Cases and Stress Tests" begin
        @testset "No Real Bidders" begin
            config = SimulationConfig(num_real_bidders = 0, num_phantom_bidders = 10, seed = 404)
        ...
        @testset "No Phantom Bidders" begin
            config = SimulationConfig(num_real_bidders = 5, num_phantom_bidders = 0, seed = 505)
    ...
    @testset "Convergence and Stability" begin
        config = SimulationConfig(num_simulations = 50, seed = 808)

        market = MarketConditions()
        simulator = PhantomSimulator(config, market)

        # Run multiple simulations with same parameters
        results = [run_simulation(simulator, 100.0, 1000.0) for _ = 1:50]
    ...
    @testset "MEV Protection Effectiveness" begin
        config = SimulationConfig(num_real_bidders = 5, num_phantom_bidders = 15, enable_vcg = true, seed = 909)
        ...
        results = [run_simulation(simulator, baseline_price, 1000.0) for _ = 1:20]
    ...
    @testset "Performance Benchmarks" begin
        ...
        config = SimulationConfig(num_real_bidders = 10, num_phantom_bidders = 30, auction_duration_ms = 100)
